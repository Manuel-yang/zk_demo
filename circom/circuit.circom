...
template ProcessTx(k) {
  ...
  for(var i = 0; i < k; i++) {
    compute_intermediate_root.path2_root_pos[i] <== sender_proof_pos[i];
  ...
  for(var i = 0; i < k; i++) {
    receiverExistence.path2_root_pos[i] <== receiver_proof_pos[i];
  ...
  for(var i = 0; i < k; i++) {
    compute_final_root.path2_root_pos[i] <== receiver_proof_pos[i];
